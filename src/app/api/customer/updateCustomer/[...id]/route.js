...
export async function PATCH(req, { params }) {
  ...
  const CustomerId = id[0]; // Access the first element of the array

  const updateData = await req.json(); // Extract the request body data (partial Customer data)

  console.log("Received Customer ID:", CustomerId); // Log the received ID
  console.log("Data to update:", updateData); // Log the update data

  // Validate the MongoDB ObjectId format
  if (!mongoose.Types.ObjectId.isValid(CustomerId)) {
    console.error("Invalid ID format:", CustomerId); // Log invalid ID format
  ...
  try {
    ...
    if (decoded.role !== "admin") {
      return NextResponse.json({ message: "Only admins can update the Customer" }, { status: 403 });
    }

    // Find Customer by ID and apply the updates
    ...
    if (!updatedCustomer) {
      console.error("Customer not found with ID:", CustomerId); // Log Customer not found
