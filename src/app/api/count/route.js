...
export async function GET() {
  try {
    ...
    const transportCount = await db.collection('transports').countDocuments({
      createdAt: { $gte: startOfDay }
    });

    // Return the counts as JSON using NextResponse
    return NextResponse.json({ inwardCount, outwardCount, transportCount });
  } catch (err) {
    console.error(err);
    // Return an error response with default values
    return NextResponse.json({ inwardCount: 0, outwardCount: 0, transportCount: 0 }, { status: 500 });
  }
}
